...
public class ErrorReporter {
    ...
    public static void print(String logFile) {
        ...
        for (Entry<String, List<Text>> entry : fileErrors.entrySet()) {
            buffer = Text.builder();
            buffer.append(Text.of(TextStyles.BOLD, TextColors.AQUA, "File: " + entry.getKey() + Files.NL));
            ...
            for (Text error : entry.getValue()) {
                String errorString = error.toPlain();
                if (errorString.startsWith(lastError, 10)) {
                    ...
                    if (similarErrors > 0) {
                        buffer.append(Text.of(TextColors.RED, "... and " + similarErrors + " more similar errors." + Files.NL));
                    }
                ...
                buffer.append(Text.of(TextColors.WHITE, error + Files.NL));
            ...
            buffer.append(Text.of(Files.NL));
            text.append(Text.of(buffer));
            Messages.info(buffer.build());
        }

        text.append(Text.of(Files.NL + Files.NL));


        if (logFile != null && Tools.saveTextToFile(text.build().toPlain(), logFile)) {
            Messages.info(Text.of(TextColors.YELLOW, "Error messages saved in" + logFile + '.'));
        }
    ...
    public static void warning(String warning, String tip) {
        entry(Text.of(TextColors.YELLOW, TextStyles.UNDERLINE, "Warning"), warning, tip);
    }
    ...
    public static boolean error(String error, String tip) {
        entry(Text.of(TextColors.RED, TextStyles.UNDERLINE, "Warning"), error, tip);
        return false;
    }

    private static void entry(Text type, String message, String tip) {
        if (!isCatching()) {
            type = Text.of(type, ":", TextColors.RESET, " " + message);

            if (tip != null) {
                type = Text.of(type, TextColors.DARK_GREEN, " TIP: ", TextColors.GRAY, tip);
            }
            ...
            type = Text.of("line " + String.format("%-5d", currentLine), type, ": ", TextColors.RESET, message);

            if (tip != null) {
                type = Text.of(type, Files.NL, TextColors.DARK_GREEN, "          TIP: ", TextColors.GRAY, tip);
            }
            errors.add(type);
